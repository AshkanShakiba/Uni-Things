...
void fgetStudent(struct student *std,FILE *info){
	char str[111];
	if(info==NULL){
	...
	strncpy(str0,str,i+1);
	sscanf(str0,"%d",&(std->id));
	j=i+1;
	while(str[j+1]!=','){
		j++;
	}
	strncpy(std->name,str+i+2,j-5);
	strcat(std->name," ");
	k=j-4;
	while(str[k+1]!=','){
		k++;
	}
	strncat(std->name,str+j+2,k-5);
}
